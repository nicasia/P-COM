...
__all__ = [
    'greedy_modularity_communities', 'greedy_modularity_communities_num',
    '_naive_greedy_modularity_communities']


def greedy_modularity_communities_num(G, num, weight=None):
...
#     print(k_for_label)
    ...
    q_cnm = modularity(G, partition, weight='weight')
    ...
    # added so we can tell the first time we would have stopped if not for the "num" parameter
    suggested_stopping=False
    while len(H) > 1:
        #print(q_cnm, len(communities))
        # Find best merge
        ...
        if dq <= 0 and not suggested_stopping:
            print("should have stopped at %i communities with modularity %.5f"%(len(communities), q_cnm))
            suggested_stopping=True
        if len(communities) <= num:
            break
            ...
            if k != j:
                ...
                for row, col in [(k, i), (i, k)]:
                    ...
                    if dq_heap[row].heap[0] == d_old:
                        ...
                        dq_heap[row].remove(d_old)
        #print([dq_dict[i].keys() for i in dq_dict.keys()])
        del dq_dict[i]
    ...
    print("Stopped at %i communities, modularity=%.5f"%(len(communities), q_cnm))
    #print(merges)
